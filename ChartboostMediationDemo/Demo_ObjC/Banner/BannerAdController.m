...
@property (nonatomic, strong, nullable) CBMBannerAdView *bannerAd;
...
- (void)loadWithViewController:(UIViewController *)viewController width:(CGFloat)width keywords:(NSDictionary<NSString *, NSString *> * _Nullable)keywords {
    ...
    self.bannerAd = [CBMBannerAdView new];
    ...
    CBMBannerAdLoadRequest *request = [[CBMBannerAdLoadRequest alloc] initWithPlacement:self.placementName size:size];
    ...
    [self.bannerAd loadWith:request viewController:viewController completion:^(CBMBannerAdLoadResult * _Nonnull result) {
...
- (void)willAppearWithBannerView:(CBMBannerAdView *)bannerView {
    // Called when a new ad is about to appear inside of `bannerAdView`. This method can be used
    // to manually size `bannerAdView` if desired.
    // This method can also be used to check other updated properties of `bannerAdView`.
}
